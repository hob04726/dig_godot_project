...
func _init() -> void:
	...
	_check(dirt.base_cost == 10, "dirt 基准价 10")
	_check(spawn.base_cost == 2500000, "spawn 基准价 2.5M")

	# 放置价序列：round(base × 1.15^n)
	var expect := [10, 12, 13, 15, 17, 20]
	...
	_check(TilePricing.refund_value(dirt, 5).to_int() == 4, "dirt 移除第 5 块返 4")
	_check(TilePricing.refund_value(dirt, 0).is_zero(), "0 块不返")
	_check(TilePricing.refund_value(spawn, 1).to_int() == 625000, "spawn 第 1 块返 625K")
	...
	# 起点价 = 基准价
	_check(TilePricing.placement_cost(spawn, 0).to_int() == 2500000, "spawn 起点价 2.5M")
